...
fn bench_auction_clear(c: &mut Criterion) {
    let port = pick_unused_port().expect("port");
    let base = format!("http://127.0.0.1:{}", port);
    let mut child = spawn_server(port);
    wait_for_health(&base, Duration::from_secs(5));

    // Seed N submissions with deterministic bids
    let n = 500;
    for i in 0..n {
        let bid = ((i * 37) % 101) as f64 + 0.5; // pseudo-randomish deterministic
        let body = json!({
    ...
    let clear_url = format!("{}/auction/clear?top_k=25", base);
    c.bench_function("auction_clear_top25", |b| {
        b.iter(|| {
            let rsp = http().post(&clear_url).send().unwrap();
            assert!(rsp.status().is_success());
...
criterion_group!(benches, bench_auction_clear);
